...
class cForum
{
	...
	//$this->form = new cForm();
	//$this->oUt->id = intval($_SESSION["ID"]);
	//$this->oUt->Leggi();
	...
	switch ($_POST["bSalva"]) {	
		...
		default:
			...
			if($this->let_id_thread()==0){
				...
				<?php
				$this->show_threads($this->aPermission);
			}else{
	...
	public function show_threads($sAllow){
		//if($sAllow["Show"]==1){
        if($this->aPermission["Show"]==1){
			...
			if ($q->Open()){
				?>
				<div style="display: block;width:100%;text-align:right;margin:0px;padding:0px;">
				<?php				
				if($sAllow["Create"]==1){
	...
	public function show_thread($sAllow){
		...
		if($sAllow["Show"]==1){
			...
			if ($q->Open() ){
				?>
				<div id="id_lista_topic" style="display: block;width:100%;text-align:right;margin:0px;padding:0px;">
				<?php				
   ...
   */	
	...
	public function show_last_reply($thread,$num){
		$q = new Query();
		$q->tables = array("messagedb");
	...
	public function insert_thread(){
		...
		$q = new Query();
		$q->tables = array("forumdb");
		$q->fields = array('SiteID' ,'Name', 'Description', 'Moderators', 'Tipo', 'Immagine', 'Ord', 'status');
		$q->values = array($this->let_id_sito(), $this->let_TitleThread(), $this->let_DescThread(), $this->let_ModThread(), $this->let_TypeThread(), $this->let_ImgThread(), $this->let_OrdThread(), $this->let_StatusThread());
		if ($q->DoInsert()){
			return true;
		} else {
			echo mysql_error();
	...
	public function update_thread(){
		...
		$q->values = array($this->let_id_sito(), $this->let_TitleThread(), $this->let_DescThread(), $this->let_ModThread(), $this->let_TypeThread(), $this->let_TypeThread(), $this->let_ImgThread(), $this->let_OrdThread(), $this->let_StatusThread());
		$q->filters = "ID = " . $this->let_id_thread();
			...
			//if ($q->Open() ){
				...
				<ol class="idPagine">		
					...
					if(((int)$Tpag)>1){
						...
						if(((int)($Tpag))==((int)$Pag) || $Tpag<=$Pmax){
						//	echo " <li><span title=\"Pagine concluse\" style=\"font-size:14px;\"><b>>></b></span><li>";
						}else{
							echo " <li><a title=\"Ci sono altre pagine.\" style=\"font-size:14px;\" href=\"" . $url . "&Offset=" . ($iC) . "\">>></a></li>";
						}
						?>
					<?php 
					}
