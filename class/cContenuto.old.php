...
class cContent extends cPage
{
	...
	private function show_cont_modify_form($txt,$img,$att,$iIdC,$imgid,$attid){
		...
		if (!empty($_POST['cmd_upload']) && $_POST['cmd_upload'] == 'Upload') {
			...
			$form->formBr();
			$form->formInput("text","Titolo",$txt->title,70,"");
			...
			$form->formBr();
			$form->formTextarea("txtSottotitolo",self::FormatStr($txt->subtitle,1),3,80,"");
			$form->formBr();
			$form->formLabel("lCorpo","Corpo del test","Corpo:","");
			$form->formBr();
			$form->formTextarea("txtCorpo",self::FormatStr($txt->body,1),20,80,"");
			$form->formBr();
			$form->formLabel("lFirma","Firma dell'autore'","Firma:","");
			$form->formBr();
			$form->formInput("text","Firma",$txt->sign,35,"");
			$form->formBr(2);
			...
			echo "<br>[noborder_'num.immagine']";
		...
		if($IdC==0){
				...
				if($k=="Modify"){
					if($value==1){
						...
						if((!is_null($pagina)) or (!$pagina=="")){
							echo "<span style='padding-right:2px;padding-left:2px;margin:0px;border-width:0px 0px 1px 0px;border-color:#3B1251;border-style:solid;'><a target='_blank' href='" . HTTP_ROOT . $pagina . "'>Gestione</a></span>";
						}
					}
				}
			}
			...
			foreach($aAllow as $k => $value){
				if($value==1){
					...
					switch($k){
						case "Create":
							echo "<span style='padding-right:2px;padding-left:2px;margin:0px;border-width:0px 0px 1px 0px;border-color:#3B1251;border-style:solid;'><a href='./index.php?action=edit&PagPadre=" . $IdPagina . "'>" . $k . "</a></span>";
							break;
						case "Modify":
							echo "<span style='padding-right:2px;padding-left:2px;margin:0px;border-width:0px 0px 1px 0px;border-color:#3B1251;border-style:solid;'><a href='./index.php?action=edit&IdP=" . $IdPagina . "'>" . $k . "</a></span>";
							break;
						case "Delete":
							echo "<span style='padding-right:2px;padding-left:2px;margin:0px;border-width:0px 0px 1px 0px;border-color:#3B1251;border-style:solid;'><a href='./index.php?action=delete&IdP=" . $IdPagina . "'>" . $k . "</a></span>";
							break;
					}
				}				
			}
