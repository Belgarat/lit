...
switch($_GET["Ord"]){
	case "":
		...
		$messaggio = "Questi sono gli ultimi dieci utenti registrati.";
		break;
	case "TUTTI":
		$sSql = "select id from userdb order by Name asc";
		$messaggio = "Di seguito tutti gli utenti iscritti.";
		break;
	default:
		$sSql = "SELECT id FROM userdb where Name like '" . $_GET["Ord"] . "%' and Abilitato=0";
		$messaggio = "Utenti che iniziano con la lettera: '".$_GET["Ord"]."'";
		break;
}
...
if (!$risultato){
	...
	//if($oUt->fControlPermission($_SERVER["SITO"],"modListaUtenti","show",$IdP)==1){
		if (mysql_num_rows($risultato)>0){
				...
				<tr>
					...
					?>					
						<td width="10">
							...
							//echo $oUt->dati["Name"];
							?><!--"><img border=0 src="/img/pbf/ico/icon1.gif"></a>-->
						</td>
					<?php
					}
					?>
					<td width="100">
					<b><font size='2'>
					<font size='2'><a style='{font-size:10px;}' href="javascript:apriUtente('<?php echo HTTP_ROOT ?>','<?php echo $oUt->id;?>')">
					<?php
					echo $oUt->dati["Name"];
					?>
					</a></font></b>
			...
			<div style="width:100%;text-align:center;"><i><?php echo $messaggio ?></i></div>
			<?php			
		}else{
