<?php
	...
	*/
	...
	class AllNewsFeed extends cSyndication{
		...
		public function writeFeed($file){
			...
			foreach ($news as $element){
				...
				$item = "<item>\n\t\t\t\t";
				$item .= "<title>".htmlentities(strip_tags($element["title"]))."</title>\n\t\t\t\t";
				$item .= "<link>".$newsLink."</link>\n\t\t\t\t";
				$item .= "<description>".htmlentities(strip_tags($element["News"]))."</description>\n\t\t\t\t";
                //var_dump($element["News"]);
				$item .= "<guid>".$newsLink."</guid>\n\t\t\t\t";
				$item .= "<category>".$this->newsType($element["id_tipo"])."</category>\n\t\t\t";
				$item .= "<pubDate>".date('r', $element["datetime"])."</pubDate>";
				$item .= "</item>\n\t\t\t";
				$item .= "<!-- ITEMS -->";
